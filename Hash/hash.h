    ...
    */
    ...
    public:
        ...
        */
        bool add(T* pElement) {
            unsigned int ind;
            if (!findLeaf(pElement, ind)) {
                leaf* new_leaf = m_Memory.newObject();
                if (!new_leaf) {
                    throw CMemoryException();
                }
                new_leaf->pnext = m_pTable[ind];
                new_leaf->pData = pElement;
                m_pTable[ind] = new_leaf;
                return true;
            }
        ...
        */
        bool update(T* pElement) {
            unsigned int ind;
            leaf* leaf_ = findLeaf(pElement, ind);
            if (!leaf_) {
                add(pElement);
                return false;
            }
        ...
        T* find(const T& element) {
            unsigned int ind;
            leaf* leaf_ = findLeaf(&element, ind);
            if (!leaf_) {
                return nullptr;
            }
        ...
        */
        bool remove(const T& element) {
            ...
            leaf* leaf_ = m_pTable[idx];
            leaf* prev_ = 0;
            while (leaf_) {
                if (!Compare(&element, leaf_->pData)) {
                    break;
                }
                prev_ = leaf_;
                leaf_ = leaf_->pnext;
            ...
            if (!prev_) {
                m_pTable[idx] = leaf_->pnext;
            } else {
                prev_->pnext = leaf_->pnext;
            }

            m_Memory.deleteObject(leaf_);
            return true;
        }
    ...
    private:
        ...
        */
        leaf *findLeaf(const T* pElement, unsigned int & idx) {
            ...
            leaf* leaf_ = m_pTable[idx];
            while (leaf_) {
                if (!Compare(pElement, leaf_->pData)) {
                    break;
                }
                leaf_ = leaf_->pnext;
            }
